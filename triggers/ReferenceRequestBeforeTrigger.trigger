trigger ReferenceRequestBeforeTrigger on roi__Reference_Request__c(before insert){
    
    Set<Id> opportunityIdSet = new Set<Id>();
    Map<Id,Id> oppSupportRequestMap = new Map<Id,Id>();
    
    for(roi__Reference_Request__c refRequest : Trigger.new)
    {
        //prepare a set with all opportunity ids where support request is empty for Reference Request
        if(refRequest.roi__Opportunity__c != null && refRequest.Support_Request__c == null)
        {
            opportunityIdSet.add(refRequest.roi__Opportunity__c);
        }
    }
    ...
    if(opportunityIdSet.size()>0)
    {
        ...
        {
            if(refRequest.roi__Opportunity__c != null && refRequest.Support_Request__c == null 
                && oppSupportRequestMap.containsKey(refRequest.roi__Opportunity__c))
            {
                refRequest.Support_Request__c = oppSupportRequestMap.get(refRequest.roi__Opportunity__c);
            }
        }
    }
}